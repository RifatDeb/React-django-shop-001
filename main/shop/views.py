...
class TrendingProduct(views.APIView):
    def get(self,request):
        query = product.objects.all().order_by('-id')[:8]
        serializers = productserializer(query,many=True,context={'request': request})
        return Response (serializers.data)
            

class LatestProduct(views.APIView):
    def get(self,request):
        query = product.objects.all()[:8]
        serializers = productserializer(query,many=True,context={'request': request})
        return Response (serializers.data)
...
class UserDataUpdate(views.APIView):
    ...
    def post(self, request):
        try:    
            ...
            user_obj.save()
            response_msg = {"error":False, "message": "Your Profile Data Is Update"}
        except:
...
class MyCart(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        for Cart in serializers.data:
            ...
            cart_product_serializer = CartproductSerializers(cart_product,many=True,context={"request":request})
